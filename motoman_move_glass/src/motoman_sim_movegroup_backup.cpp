...
void poseCallback()
{
    ...
    const Eigen::Affine3d &link_state_mh50_0 = current_state_mh50->getGlobalLinkTransform(move_group_mh50.getLinkNames()[0]);
    const Eigen::Affine3d &link_state_mh50_1 = current_state_mh50->getGlobalLinkTransform(move_group_mh50.getLinkNames()[1]);
    const Eigen::Affine3d &link_state_mh50_2 = current_state_mh50->getGlobalLinkTransform(move_group_mh50.getLinkNames()[2]);
    const Eigen::Affine3d &link_state_mh50_3 = current_state_mh50->getGlobalLinkTransform(move_group_mh50.getLinkNames()[3]);
    const Eigen::Affine3d &link_state_mh50_4 = current_state_mh50->getGlobalLinkTransform(move_group_mh50.getLinkNames()[4]);
    const Eigen::Affine3d &link_state_mh50_5 = current_state_mh50->getGlobalLinkTransform(move_group_mh50.getLinkNames()[5]);
    std::vector<double> link_state_mh50_ori;

    const Eigen::Affine3d &end_effector_state_mh5 = current_state_mh50->getGlobalLinkTransform(move_group_mh5.getEndEffectorLink());

    move_group_mh50.setPlannerId("TRRT");
    //move_group_mh50.setPlanningTime(45);
    //move_group_mh5.setPlannerId("RRTConnect");
    //move_group_mh5.setPlanningTime(45);

    move_group_mh50.allowReplanning(1);
    ...
    bool success = false;
    std::map<std::string, double> jointValue;
    jointValue[move_group_mh50.getJointNames()[0]] = 0;
    jointValue[move_group_mh50.getJointNames()[1]] = 0;
    jointValue[move_group_mh50.getJointNames()[2]] = 0;
    jointValue[move_group_mh50.getJointNames()[3]] = 0;
    jointValue[move_group_mh50.getJointNames()[4]] = 0;
    jointValue[move_group_mh50.getJointNames()[5]] = 0;
    ...
    for (std::size_t i = 0; i < trajectory.joint_trajectory.points.size(); ++i){
        ...
        for(std::size_t j = 0; j < joint_names_mh50.size(); ++j){
            joint_group_positions_mh50[j] = trajectory.joint_trajectory.points[i].positions[j];
            current_state_mh50->setJointGroupPositions(joint_model_group_mh50, joint_group_positions_mh50);
            current_state_mh50->getGlobalLinkTransform(move_group_mh50.getLinkNames()[j]);
            current_state_mh50->updateLinkTransforms();
            current_state_mh50->update();
